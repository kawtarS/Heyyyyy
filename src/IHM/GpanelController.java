...
public class GpanelController implements Initializable {
    ...
    @FXML
    private ImageView btn_Monitoring;
...
try{btn_voiture.getScene().getWindow().hide();
 ...
 try{btn_Emp.getScene().getWindow().hide();
    ...
    @FXML
    private void switchMonitiring(MouseEvent event) {
        //add 
        try{btn_Monitoring.getScene().getWindow().hide();
                Parent root=FXMLLoader.load(getClass().getResource("hiiiii.fxml"));//your file 
                Stage mainStage=new Stage();
                Scene scene=new Scene(root);
                mainStage.setScene(scene);
                mainStage.show();
